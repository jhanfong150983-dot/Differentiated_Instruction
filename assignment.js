...
(function() {
            ...
            <div class="card-body">
                ...
                <div class="course-selector" style="margin-bottom: var(--spacing-md);">
                    ...
                    >
                        ...
                        ${(() => {
                            APP_CONFIG.log('🔍 渲染課程下拉選單:', {
                                classId: classData.classId,
                                coursesCount: assignmentCourses.length,
                                courses: assignmentCourses
                            });
                            return assignmentCourses.map(course => {
                            let displayName = escapeHtml(course.name || course.courseName);
                            if (!course.isOwner && course.sharedBy) {
                                displayName += ` (由 ${course.sharedBy} 創建)`;
                            }
                            return `
                                <option value="${course.course_id || course.courseId}" ${(course.course_id || course.courseId) === classData.courseId ? 'selected' : ''}>
                                    ${displayName}
                                </option>
                            `;
                            }).join('');
    ...
    window.saveAssignment = function(classId) {
        ...
        // 找到課程名稱（容錯處理：支援 courseId 和 course_id）
        const course = assignmentCourses.find(c => (c.courseId || c.course_id) === courseId);
        if (!course) {
            APP_CONFIG.error('找不到課程資訊', { courseId, availableCourses: assignmentCourses });
        ...
        const classData = assignmentClasses.find(c => c.classId === classId);
        if (!classData) {
            APP_CONFIG.error('找不到班級資訊', { classId, availableClasses: assignmentClasses });
            showToast('找不到班級資訊', 'error');
            return;
        }

        // 取得課程名稱（容錯處理：支援 courseName 和 name）
        const courseName = course.courseName || course.name;

        // 確認對話框
        if (!confirm(`確定要將課程「${courseName}」安排給班級「${classData.className}」嗎？\n\n此操作會取代該班級原有的授課安排（如果有的話）。`)) {
            return;
        }
        ...
        fetch(`${APP_CONFIG.API_URL}?${params.toString()}`)
            ...
            .then(function(response) {
                ...
                if (response.success) {
                    showToast(`✅ 成功安排課程「${courseName}」給班級「${classData.className}」`, 'success');
